...
volatile uint32_t *tim3 = (uint32_t*) TIM3;
...
void speaker_init(void) {
	volatile uint32_t *rcc = (uint32_t*) RCC_ENR;
	*rcc |= GPIOBEN;

	gpiob[MODER_INDEX] &= ~(0b11<<(4*2)); //clear
	gpiob[MODER_INDEX] |= (0b10<<(4*2)); //alternate mode

	gpiob[AFRL_INDEX] &= ~(0b1111<<(4*4));
	gpiob[AFRL_INDEX] |= (0b0010<<(4*4)); // TIM3_CH1

	volatile uint32_t *rcc_tim = (uint32_t*) RCC_TIM_ENR;
	*rcc_tim |= TIM3_EN;
